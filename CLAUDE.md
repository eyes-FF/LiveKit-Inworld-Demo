...
cd api && ../.venv/bin/uvicorn main:app --port 8000        # API
.venv/bin/python agent/main.py dev                          # Agent
cd frontend && npm run dev                                  # 前端(3000 被占会落到 3001)

# 全链路自动化验证(说中文给 agent 听,校验回复与转写)
.venv/bin/python -u scripts/talk_probe.py /tmp/probe_say.wav

# 升级线上
git push                          # 后端:Railway 自动构建(~10min)
cd frontend && vercel --prod      # 前端:CLI 直推(~30s),不走 GitHub!
```
...
改动 agent/api 后用 `scripts/talk_probe.py` 跑全链路;改前端用浏览器实测(开始通话 → 开场白字幕 → 设置面板锁定)。生产部署后 curl `/health` + 探针 `BACKEND_URL=<railway域名> scripts/probe_client.py`。
